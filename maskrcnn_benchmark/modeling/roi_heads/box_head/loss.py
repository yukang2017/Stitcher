...
class FastRCNNLossComputation(object):
    ...
    def subsample(self, proposals, targets, ratio_stitch):
        """
        ...
        labels, regression_targets, matched_targets_boxes = self.prepare_targets(proposals, targets)
        sampled_pos_inds, sampled_neg_inds = self.fg_bg_sampler(labels, ratio_stitch)

        proposals = list(proposals)
        # add corresponding label and regression_targets information to the bounding boxes
        for labels_per_image, regression_targets_per_image, proposals_per_image, matched_targets_boxes_per_image in zip(
            labels, regression_targets, proposals, matched_targets_boxes
        ):
            ...
            proposals_per_image.add_field(
                "matched_targets_boxes", matched_targets_boxes_per_image
            )
    ...
    def __call__(self, class_logits, box_regression):
        ...
        matched_targets_boxes = cat(
            [proposal.get_field("matched_targets_boxes") for proposal in proposals], dim=0
        )
